...
animal = Animal("green", "rrr")  # создаём объект - теперь это большая переменная, которая умеет делать больше, чем обычные
print(animal.color)  # можем просто выписать любой параметр
print(animal.do_voice())  # можем вызывать функции, который есть в класса объекта
...
class Tiger(Animal):  # создаём подкласс - более точная модификация класса
    ...
    def do_voice(self):  # можем изменить предыдущую фунцкию или создать новую, в Animal или в других подкалассах будет другое
